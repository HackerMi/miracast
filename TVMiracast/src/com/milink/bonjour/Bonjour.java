...
public class Bonjour implements Runnable, ServiceListener {
    ...
    public void publishService(BonjourServiceInfo svcInfo) {
        Log.d(TAG, String.format("publishService"));
        ...
        synchronized (mJmdnsLock) {
            ...
            try {
                ...
                if (ip != null) {
                    ...
                    Log.d(TAG, String.format("DHCP client mode: %s", addr.getHostAddress()));
                }
                else {
                    String strIp = NetWork.getWifiApIpAddress();
                    if (strIp == null) {
                        Log.d(TAG, "NetWork.getWifiApIpAddress failed!");
                        mListener.onStartFailed();
                        return;
                    }
                    addr = InetAddress.getByName(strIp);
                    Log.d(TAG, String.format("AP mode: %s", addr.getHostAddress()));
                }
